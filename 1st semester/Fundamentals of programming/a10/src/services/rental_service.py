...
from src.struct.iterable_structure import sort_list, filter_list
...
class RentalService:
    ...
    def return_book(self, rental_id, ret_date):
        ...
        undo = FunctionCall(self.delete_ret_date, rental_id)
        redo = FunctionCall(self.rental.return_book, rental_id, ret_date)
        op = Operation(undo, redo)
        self.undo_controller.record(op)

    def delete_ret_date(self, rental_id):
        """
        function used for undo
        :param rental_id:
        :return:
        """
        rental = self.rental.find_by_rental_id(rental_id)
        rental.returned_date = ''

    def verif_list(self):
        if len(self.rental.rentals._list) == 0:
    ...
    def most_active_clients(self, _list):
        ...
        # Sort the list
        # lambda <parameter list> : <function body>
        sort_list(result, lambda a, b: a.rental_days > b.rental_days)
    ...
    def most_rented_author(self, _list):
        ...
        for rental in self.rental.rentals:
            a = self.find_in_list(_list, rental.book_id)
            if a != -1:
