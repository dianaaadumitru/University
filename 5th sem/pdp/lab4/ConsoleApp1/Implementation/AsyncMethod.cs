...
public static class AsyncMethod
{
    ...
    private static async void StartClient(string host, int id)
    {
        ...
        await ConnectWrapper(socket);

        // request data from the server
        await SendWrapper(socket, HttpParser.GetRequestString(socket.hostname, socket.endpoint));

        // receive the response from the server
        await ReceiveWrapper(socket);
    ...
    private static async Task ConnectWrapper(CustomSocket socket)
    {
        socket.socket.BeginConnect(socket.remoteEndpoint, ConnectCallback, socket);

        await Task.FromResult<object>(socket.connectDone.WaitOne());
    }
    ...
    private static void ConnectCallback(IAsyncResult ar)
    {
        ...
        Console.WriteLine("AsyncAwait - {0} --> Socket connected to {1} ({2})", clientId, hostname, urlPath);

        // signal that the connection has been made 
        socket.connectDone.Set();
    }

    private static async Task SendWrapper(CustomSocket socket, string data)
    {
        ...
        await Task.FromResult<object>(socket.sendDone.WaitOne());
    }
    ...
    private static void SendCallback(IAsyncResult ar)
    {
        ...
        Console.WriteLine("AsyncAwait - {0} --> Sent {1} bytes to server.", clientId, bytesSent);

        // signal that all bytes have been sent
        socket.sendDone.Set();
    }

    private static async Task ReceiveWrapper(CustomSocket socket)
    {
        ...
        await Task.FromResult<object>(socket.receiveDone.WaitOne());
    }
